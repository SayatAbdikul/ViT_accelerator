"""W8A32 code generator: parallel implementation of :mod:`codegen` for
the FP32-activation, INT8-weight-fake-quant path.

Key differences from :class:`codegen.CodeGenerator`:

1. **Dequantized FP32 weights.** Weights are stored in DRAM as FP32 tensors
   that already carry the INT8 rounding error (``w_int8 * w_scale``). The
   systolic array then does FP32 × FP32 → FP32 MATMUL, matching the
   ``fake_quant.apply_weight_quantization`` reference exactly.

2. **No REQUANT / REQUANT_PC / DEQUANT_ADD / SCALE_MUL clip ops.** All
   activations flow as FP32 end-to-end; the scale-management plumbing
   that bridges INT8 intermediates in W8A8 has no role.

3. **ABUF tiles are 4 bytes/element.** Every allocation that holds
   activations multiplies the per-tile element count by 4.

4. **Always seq-tiled.** The W8A32 memory budget forces tiling for both
   DeiT-tiny (FP32 residual = 156 KB > 128 KB ABUF) and ViT-B (624 KB), so
   the codegen only implements the tiled IR ops (``init_residual_tile``,
   ``tile_load``, ``tile_save``, ``concat_heads_tile`` and the per-tile
   versions of matmul/SFU/vadd).

5. **No trace manifest, no fused-path optimizations.** The W8A32 fork has
   no RTL parity gate; tracing is a structural no-op. Fused paths
   (gelu_from_accum, dequant_add_residual1, fused_softmax_attnv,
   REQUANT_PC variants) all assume INT8 bridging and are not honored.

6. **Bias added via VADD broadcast in ACCUM.** Biases are stored as FP32
   row vectors in DRAM; after MATMUL they are broadcast-added to the
   FP32 ACCUM, then moved to ABUF via a flat BUF_COPY (which preserves
   the FP32 bit pattern through the int32-typed ACCUM bytes).
"""
...
class CodeGeneratorW8A32:
    """Generate W8A32 ISA instructions from a seq-tiled IR graph.

    Inputs:
      * ``fp32_weight_data``: ``{name: fp32_ndarray}`` of dequantized FP32
        weights pre-padded and pre-transposed to the systolic layout
        ([K_pad, N_pad] for 2D weights). LayerNorm gamma/beta are stored
        as FP16 (matching the SFU convention).
      * ``fp32_biases``: ``{name: fp32_ndarray}`` of FP32 bias row vectors
        already padded to ``N_pad`` elements.
    ...
    def __init__(self,
        ...
        # tile_load nodes whose only consumer is matmul_attn_v get their DMA
        # deferred until inside _emit_attn_v so Q + K + V never coexist in
        # ABUF (each is ~53 KB FP32 and total exceeds 128 KB ABUF).
        self._deferred_loads: Dict[str, IRNode] = {}
        # Populated by ``generate`` so callers can fetch the FP32 logit row.
    ...
    def _mark_deferred_loads(self, graph: IRGraph):
        """Identify tile_load nodes whose only consumer is matmul_attn_v.

        Those nodes (typically ``block{i}_head{h}_value``) carry V tensors
        sized 53 KB FP32 each; loading them eagerly forces Q+K+V to coexist
        in ABUF, which overflows the 128 KB budget. We defer their DMA
        until ``_emit_attn_v`` runs.
        """
        ...
        for node in graph.nodes:
            if node.op != "tile_load":
                continue
            users = consumers.get(node.name, [])
            if len(users) == 1 and users[0].op == "matmul_attn_v":
                self._deferred_loads[node.name] = node
    ...
    def _layout_weights(self, graph: IRGraph):
        ...
        for name, bias in self.fp32_biases.items():
            self.dram_layout[name] = offset
            blob = bias.astype(np.float32).tobytes()
            self.dram_blob.extend(blob)
            offset += len(blob)
        # Zero-pad for K/V attention masking (FP32 zeros).
        ...
        # of ≈ 1/Z. Without this the CLS row leaks ~5% of probability to the
        # 11 zero-padded positions, shrinking AttnV by the same factor every
        # block and compounding to a ~0.02 cosine deficit vs fake_quant.
        attn_mask = np.zeros(self.cfg.seq_len_pad, dtype=np.float32)
        attn_mask[self.cfg.seq_len:] = -1e9
        self.dram_layout["__attention_mask__"] = offset
        mask_blob = attn_mask.tobytes()
        self.dram_blob.extend(mask_blob)
        offset += len(mask_blob)
        # Input patches placeholder (FP32 patch embeddings written by host).
    ...
    def _accum_to_abuf(self, M_pad: int, N_pad: int, dst_off_units: int):
        """Copy an FP32 [M_pad, N_pad] tile from ACCUM (offset 0) to ABUF.

        The byte payload is identical (ACCUM stores int32 bytes; for W8A32
        those bytes encode FP32), so a flat BUF_COPY moves the data
        without value change.
        """
        total_bytes = M_pad * N_pad * ELEM_BYTES
        length_units = total_bytes // UNIT
        self._emit(BufCopyInsn(
            src_buf=BUF_ACCUM, src_off=0,
    ...
    def _emit_matmul(self, node: IRNode):
        ...
        for k in range(num_strips):
            ...
            if num_strips == 1:
                # ACCUM [M_pad, N_pad] → ABUF as one flat copy.
                self._accum_to_abuf(M_pad, N_pad, out_alloc.offset_units)
            else:
                # ACCUM [M_pad, N_strip] → ABUF at column window n_start.
                # One BUF_COPY per output row (length = N_strip * 4 / 16 units).
                ...
                for r in range(M_pad):
                    src_off = (r * N_strip * ELEM_BYTES) // UNIT
                    dst_off = out_alloc.offset_units + r * row_units_full + col_offset_units
                    self._emit(BufCopyInsn(
                        src_buf=BUF_ACCUM, src_off=src_off,
