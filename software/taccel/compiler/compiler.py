...
Modes
-----
- ``"w8a8"`` (default) — INT8 weights, INT8 activations, INT32 accumulator.
  Production path; matches the RTL bit-for-bit on the frozen image set
  (see ``software/tools/batch_compare_rtl_golden.py:141``).
- ``"w8a32"`` — INT8 weights, FP32 activations, FP32 accumulator. Used to
  prove the weight-quant accuracy ceiling end-to-end through the real
  toolchain (no RTL parity in this mode; the bit-exact gate is suspended).
  The W8A32 compile path is implemented in
  ``software/taccel/compiler/codegen_w8a32.py`` and the matching simulator
  in ``software/taccel/golden_model/simulator_w8a32.py``.
"""
import struct
import numpy as np
...
from ..quantizer.quantize import quantize_weights, quantize_tensor, dequantize_tensor
...
class Compiler:
    ...
    def __init__(self, cfg: Optional[ModelConfig] = None,
                 mode: Literal["w8a8", "w8a32", "w8a16"] = "w8a8"):
        if mode not in ("w8a8", "w8a32", "w8a16"):
            raise ValueError(
                f"mode must be 'w8a8', 'w8a32', or 'w8a16', got {mode!r}"
            )
    ...
    def compile_w8a32(self, state_dict: dict) -> ProgramBinary:
        ...
        for name, tensor in state_dict.items():
            ...
            # cls_token / position_embeddings arrive as ndim=3 ([1, 1, embed]
            # / [1, seq, embed]). Squeeze and pad them as FP32 row tensors so
            # init_residual_tile can DMA them directly.
            ...
            if 'weight' in name and arr.ndim >= 2:
                ...
                # Per-channel quantize → dequantize: w_fp32_dq = w_int8 * w_scale[c]
                q, scales = quantize_tensor(arr, per_channel=True)
                w_dq = dequantize_tensor(q, scales).astype(np.float32)
                # Pad to multiples of 16 then transpose to [K_pad, N_pad].
                w_dq = np.pad(
                    w_dq,
                ...
                if weight_name in state_dict:
                    ...
                    if hasattr(w, 'numpy') and w.numpy().ndim >= 2:
                        # Matmul/Conv2d bias — pad to 16-multiple FP32.
        ...
        # ── per-head Q/K/V weight + bias slices ───────────────────────
        for layer_idx in range(self.cfg.depth):
            prefix = f"{prefix_root}.encoder.layer.{layer_idx}"
            for proj in ("query", "key", "value"):
                wname = f"{prefix}.attention.attention.{proj}.weight"
        ...
        # ── IR construction + pass pipeline (W8A32 variant) ───────────
        graph = extract_vit_graph(self.cfg)
        ctx: Dict[str, Any] = {}
        ...
        # Patch SET_ADDR_LO immediates to make all DRAM addresses absolute.
        patched = bytearray(insn_bytes)
        for i in range(0, len(patched), 8):
            word = struct.unpack(">Q", patched[i:i + 8])[0]
            opcode_val = (word >> OPCODE_SHIFT) & OPCODE_MASK
        ...
        if pos_emb_dram_start is not None:
            pos_embed_cls_dram_offset = data_base + pos_emb_dram_start
            # Each pos_embed row is embed_dim FP32 values (4 bytes/element).
            pos_embed_patch_dram_offset = (
                data_base + pos_emb_dram_start + self.cfg.embed_dim * 4
            )
        else:
            pos_embed_cls_dram_offset = 0
            pos_embed_patch_dram_offset = 0
        ...
        return ProgramBinary(
            ...
            compiler_manifest={
                "manifest_version": 1,
                "compiler": {"class": self.__class__.__name__, "mode": "w8a32"},
                "program_layout": {
                    "data_base": int(data_base),
                    "input_offset": int(input_offset),
                    "pos_embed_patch_dram_offset": int(pos_embed_patch_dram_offset),
    ...
    def compile_w8a16(self, state_dict: dict) -> ProgramBinary:
        ...
        if pos_emb_dram_start is not None:
            pos_embed_cls_dram_offset = data_base + pos_emb_dram_start
            # Each pos_embed row is embed_dim FP16 values (2 bytes/element).
            pos_embed_patch_dram_offset = (
                data_base + pos_emb_dram_start + self.cfg.embed_dim * 2
            )
        else:
            pos_embed_cls_dram_offset = 0
            pos_embed_patch_dram_offset = 0
        ...
        return ProgramBinary(
            ...
            compiler_manifest={
                "manifest_version": 1,
                "compiler": {"class": self.__class__.__name__, "mode": "w8a16"},
                "program_layout": {
                    "data_base": int(data_base),
                    "input_offset": int(input_offset),
                    "pos_embed_patch_dram_offset": int(pos_embed_patch_dram_offset),
