"""Integration tests for the W8A16 compile + simulate path.

These tests live alongside the W8A8 / W8A32 suites and run only when the
DeiT-tiny weights are reachable (the conftest gate). They prove:

1. ``Compiler(mode='w8a16').compile_w8a16(state_dict)`` emits a
   ProgramBinary whose instruction stream contains no INT8 quant-bracket
   opcodes (REQUANT / REQUANT_PC / DEQUANT_ADD / SOFTMAX_ATTNV).
2. ``SimulatorW8A16`` can execute the full program to HALT on synthetic
   pixel inputs without raising.
3. The classifier logits land in ABUF at the offset recorded in
   ``compiler_manifest['classifier_output']`` and are non-trivial FP16.
4. The load-bearing accuracy gates: the W8A16 toolchain reproduces the
   ``fake_quant`` ceiling within FP16-narrowed numerical tolerance
   (``cosine ≥ 0.998`` vs ``apply_weight_quantization(model)`` forward),
   and the cosine vs the FP32 reference clears the ≥ 0.997 plan threshold.
"""
...
def test_compile_w8a16_no_int8_quant_opcodes():
    """W8A16 program must not contain INT8-quant-bracket instructions."""
    model = _load_deit_tiny()
    c = Compiler(cfg=ModelConfig.deit_tiny(), mode="w8a16")
    program = c.compile_w8a16(model.state_dict())
    ...
    assert not bad, (
        f"W8A16 program contains INT8 quant-bracket opcodes: {bad}"
    )


def test_compile_w8a16_classifier_output_recorded():
    ...
    assert program.compiler_manifest["compiler"]["mode"] == "w8a16"


def test_compile_w8a16_seq_tiling_matches_policy():
    """W8A16 DeiT-tiny still tiles (FC1 cap forces it) at tile_rows=16."""
    model = _load_deit_tiny()
    c = Compiler(cfg=ModelConfig.deit_tiny(), mode="w8a16")
    program = c.compile_w8a16(model.state_dict())
    st = program.compiler_manifest["seq_tiling"]
    assert st["needs_tiling"] is True
    assert st["tile_rows"] == 16
    assert st["num_tiles"] == 13


def test_compile_w8a16_end_to_end_runs_to_halt():
    ...
    # in-program FP16-dequant weights for the rest of the network. The host
    # convolution stays FP32 for stability; we narrow to FP16 only at the
    # DMA boundary (matches the W8A16 input contract).
    ...
    assert sim.state.halted, "W8A16 program did not reach HALT"

    co = program.compiler_manifest["classifier_output"]
    logits = np.frombuffer(
        sim.state.abuf, dtype=np.float16,
        count=co["N_pad"], offset=co["offset_bytes"],
    )[:co["logical_cols"]].astype(np.float32)
    assert np.isfinite(logits).all(), "W8A16 produced NaN/Inf logits"
    assert np.any(logits != 0.0), "W8A16 produced all-zero logits"
    ...
    # Load-bearing accuracy gates from the W8A16 plan:
    # * cos_fq ≥ 0.998 — the W8A16 toolchain must reproduce the fake_quant
    #   reference within tight tolerance. FP16 narrowing on every ABUF
    #   write adds a ~3-decimal-digit per-tensor rounding noise, so the
    #   gate is one nine looser than W8A32's 0.999 bit-equivalence bar.
    # * cos_fp32 ≥ 0.997 — the W8A16 toolchain must clear the FP32
    #   reference within the same envelope the fake_quant ceiling clears
    #   (~0.9995); one nine of headroom for FP16 narrowing compounded
    #   across 12 transformer blocks.
    assert cos_fq >= 0.998, (
        f"W8A16 cosine vs fake_quant = {cos_fq:.6f} below the 0.998 gate; "
        f"a regression has reintroduced numerical drift between the W8A16 "
        f"toolchain and the fake_quant reference."
    )
    assert cos_fp32 >= 0.997, (
        f"W8A16 cosine vs FP32 = {cos_fp32:.6f} below the 0.997 plan gate"
    )
